...
arm6 = 2*arm3+D_cog6_vec;
arm7 = 2*arm3+D_cog7_vec;
% speed vectors
...
mass = [m2,m3,m4,m5,m6,m7];
inertia = [J2,J3,J4,J5,J6,J7];
arm = [arm2(k,:);arm3(k,:);arm4(k,:);arm5(k,:);arm6(k,:);arm7(k,:)];
vel = [vel_2(k,:);vel_3(k,:);vel_4(k,:);vel_5(k,:);vel_6(k,:);vel_7(k,:)];
acc = [acc_2(k,:);acc_3(k,:);acc_4(k,:);acc_5(k,:);acc_6(k,:);acc_7(k,:)];
omega = [omega2(k,3),omega3(k,3),omega4(k,3),omega5(k,3),omega6(k,3),omega7(k,3)];
alpha = [alpha2(k,3),alpha3(k,3),alpha4(k,3),alpha5(k,3),alpha6(k,3),alpha7(k,3)];
for i=1:6
...
%     axis tight
    ...
    subplot(2,1,1)
    plot(t,Fshakx);
    ylabel('Shaking Force X-direction')
    xlabel('t')
    subplot(2,1,2)
    plot(t,Xtest)
    ylabel('Absolute error')
    ...
    subplot(2,1,1)
    plot(t,Fshaky);
    ylabel('Shaking Force Y-direction')
    xlabel('t')
    subplot(2,1,2)
    plot(t,Ytest)
    ylabel('Absolute error')
